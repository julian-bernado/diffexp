...
numeric_summary <- data.frame(
  feature = colnames(merged_df)[numeric_columns],
  mean = sapply(merged_df[, numeric_columns], mean, na.rm = TRUE),
  sd = sapply(merged_df[, numeric_columns], sd, na.rm = TRUE),
  var = sapply(merged_df[, numeric_columns], var, na.rm = TRUE)
)

# Display top 10 variables with the highest standard deviation
top_num <- numeric_summary %>%
  arrange(desc(sd)) %>%
  head(10)
# Assuming 'top_num' contains the top 10 features with the highest variability
top_features <- top_num$feature

# Gather the data for the selected features into long format
data_long <- merged_df %>%
  select(all_of(top_features)) %>%
  pivot_longer(cols = everything(), names_to = "feature", values_to = "value")

# Create the ggplot with faceting
random_features_hist <- ggplot(data_long, aes(x = value)) +
  geom_histogram(bins = 20, fill = "lightblue", color = "black") +
  facet_wrap(~ feature, nrow = 2, ncol = 5, scales = "free_x") +
  theme_minimal() +
  labs(x = "Value", y = "Frequency", title = "Histograms of Top 10 Variables with Highest Variability")
```

```{r}
# Identify gene expression columns
gene_cols <- colnames(merged_df)[!(colnames(merged_df) %in% c("patient", "sex", "region", "lab", "chip.version"))]

# Create a function to summarize gene expression by sex for each lab
get_lab_expression_by_sex <- function(lab_name) {
  merged_df %>%
    filter(lab == lab_name) %>%
    group_by(sex) %>%
    summarise(across(all_of(gene_cols), mean, na.rm = TRUE)) %>%
    pivot_longer(-sex, names_to = "gene", values_to = "mean_expression") %>%
    mutate(lab = lab_name)  # Add lab name for identification
}

# Get the summarized gene expression data for each lab
davis_expr <- get_lab_expression_by_sex("Davis")
irvine_expr <- get_lab_expression_by_sex("Irvine")
michigan_expr <- get_lab_expression_by_sex("Michigan")

# Combine the data from all labs
combined_expr <- bind_rows(davis_expr, irvine_expr, michigan_expr)

# Create a histogram to visualize the gene expression for males and females across labs
mean_gene_before <- ggplot(combined_expr, aes(x = mean_expression, fill = lab)) +
  geom_histogram(binwidth = 0.1, position = "identity", alpha = 0.6) +  # Set alpha to distinguish overlapping bins
  facet_wrap(~ sex, scales = "free_y") +  # Separate histograms by sex
  labs(title = "Gene Expression by Sex Across Labs",
       x = "Mean Gene Expression",
       y = "Frequency",
       fill = "Lab") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("yellow", "green", "blue"))  # Adjust colors as needed
```

```{r}
# Identify gene expression columns
gene_cols <- colnames(normalized_data )[!(colnames(normalized_data ) %in% c("patient", "sex", "region", "lab", "chip.version"))]
...
# Create a histogram to visualize the gene expression for males and females across labs
mean_gene_after <- ggplot(combined_expr, aes(x = mean_expression, fill = lab)) +
  geom_histogram(binwidth = 0.1, position = "identity", alpha = 0.6) +  # Set alpha to distinguish overlapping bins
  facet_wrap(~ sex, scales = "free_y") +  # Separate histograms by sex
  labs(title = "Gene Expression by Sex Across Labs after Normalization",
       x = "Mean Gene Expression",
       y = "Frequency",
       fill = "Lab") +
  theme_minimal() +
...
```{r, mean_gene_before, fig.width = 8, fig.height = 5, fig.cap = "Histogram of the Gene Expression for males and females across labs"}}
mean_gene_before
```

```{r, random_features_hist, fig.width = 8, fig.height = 5}
random_features_hist
```

```{r, mean_gene_after, fig.width = 8, fig.height = 5,}
mean_gene_after
```

Normalization is a crucial step in this data analysis. Based on the graphical representation Figure \@ref(fig:random_features_hist) of several randomly selected features, we observe that none of them exhibit a unimodal distribution. For most features, two or three distinct peaks are visible in the histograms. This initial observation prompts a closer examination of the data to uncover potential underlying causes. One hypothesis is that the bimodality in the data could be attributed to sex or chip version, as both of these features have two distinct values. When we observe three distinct peaks, they could potentially be explained by the three lab values or the three brain regions. However, chip version and lab names are not desirable explanations for these observed differences. Therefore, we aim to normalize the data to eliminate any dependency between gene expression distributions and either chip version or lab. We apply normalization to address several key issues:

-   **Correcting for Technical Variability**

Technical factors, such as lab-specific protocols and differences between chip versions, introduce variability that is unrelated to the biological signals of interest. Normalization reduces this variability, ensuring that observed differences between samples are driven by biological factors, not technical artifacts.

-   **Handling Systematic Bias**

Irvine lab consistently produces higher expression values than Davis and Michigan probably due to factors unrelated to biology, such as differences in equipment or experimental conditions. Normalization compensates for these biases so that the analysis can focus on meaningful biological variation, not systematic bias.

-   **Ensuring Comparability Across Samples**

Different samples may have varying overall intensities due to uncontrolled factors like RNA quantity, labeling efficiency, or the number of cells in a tissue sample. Without normalization, comparing gene expression levels across samples would not be fair or reliable. Normalization helps to adjust for these differences, allowing for meaningful comparisons across all samples.

-   **Supporting Statistical Testing**

Normalization is essential for applying statistical tests such as the Wilcoxon test or linear models, which assume that the data is comparable across groups. Without normalization, these tests could lead to false conclusions, as uncorrected technical variability could skew the results.

The success of normalization depends on the method used and how well it addresses the underlying variability. Thus, we decided to try several normalization methods:

-   Mean Normalization for Each Lab Separately

-   Mean Normalization (Global)

-   Z-Score Normalization (group by lab and by chip version)
...
### 1. Mean Normalization for Each Lab Separately

-   Idea: Subtract the mean gene expression of each lab from the data and divide by the standard deviation within each lab. This normalizes each lab’s data to have zero mean and unit variance.
-   Advantages: Controls for lab-specific variability, helping remove lab effects while retaining biological differences within each lab.
-   Disadvantages: It doesn't account for global variation across labs, so comparisons between labs might still reflect technical variability.

### 2. Mean Normalization (Global)

-   Idea: The mean of all samples (across all labs) is subtracted from each data point, and the data is rescaled using global standard deviation.
-   Advantages: Provides global consistency, allowing comparisons across labs.
-   Disadvantages: Not account for lab-specific technical differences, introducing bias since labs have systematic variations.

### 3. Z-Score Normalization (Grouped by Lab and Chip Version)

-   Idea: Standardizes gene expression within each lab and chip version by subtracting the mean and dividing by the standard deviation of each group.
-   Advantages: Balances normalization by both lab and chip version, reducing the confounding effects of technical artifacts from both sources.
-   Disadvantages: Can complicate interpretation, especially if the chip version or lab introduces non-linear effects, and might mask biological differences.

### 4. Spike-in Normalization

-   Idea: Normalization is based on control genes (spike-ins) that are added in known quantities, adjusting all samples relative to these controls.
-   Advantages: Corrects for technical variability using known controls, providing a strong base for cross-sample comparisons.
-   Disadvantages: Assumes that spike-ins accurately reflect the behavior of endogenous genes, which might not always hold true, especially if spike-ins behave differently from other genes.

### 5. ComBat

-   Idea: A method commonly used for batch effect correction, ComBat adjusts for systematic differences between batches (in this case, labs or other factors) while retaining biological variability.
-   Advantages: Effective in removing batch effects while preserving true biological differences.
-   Disadvantages: There is potential for over-correction, which might remove some biological variation along with technical variation.

### 6. Quantile Normalization

-   Idea: Forces the distribution of gene expression values to be the same across all samples by sorting and averaging ranks.
-   Advantages: Ensures comparability across samples, removing large-scale differences in distribution.

Among the methods we tried, Quantile Normalization and a combination of Quantile Normalization with ComBat performed best at reducing unwanted variability. Quantile normalization ensures that the distribution of gene expression values across samples is identical, making any differences more likely to be biological rather than technical in nature and ComBat normalization further mitigates lab-specific effects. ComBat corrects for batch effects that stem from differences in experimental conditions or equipment. It uses an empirical Bayes approach to adjust for these batch differences without eliminating the biological variation we aim to study.

These two techniques provided the most consistent and reliable results Figure \@ref(fig:mean_gene_before) \@ref(fig:mean_gene_after), which is why we selected them for further analysis.
